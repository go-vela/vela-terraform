...
func TestTerraform_Init_Command(t *testing.T) {
	...
	want := exec.Command(
		...
		initAction,
		"-backend=true",
		fmt.Sprintf("-backend-config=%s -backend-config=%s",
			i.InitOptions.BackendConfigs[0], i.InitOptions.BackendConfigs[1]),
		"-force-copy",
		...
		"-no-color",
		fmt.Sprintf("-plugin-dir=%s -plugin-dir=%s", i.InitOptions.PluginDirs[0], i.InitOptions.PluginDirs[1]),
		"-reconfigure",
		"-upgrade=false",
		"-verify-plugins=true",
		i.Directory,
