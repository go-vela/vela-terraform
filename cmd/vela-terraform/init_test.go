...
func TestTerraform_Init_Command(t *testing.T) {
	...
	want := exec.Command(
		...
		initAction,
		"-backend=true",
		fmt.Sprintf("-backend-config=%s -backend-config=%s", i.BackendConfigs[0], i.BackendConfigs[1]),
		"-force-copy",
		fmt.Sprintf("-from-module=%s", i.FromModule),
		"-get=true",
		"-get-plugins=true",
		"-input=true",
		"-lock=true",
		fmt.Sprintf("-lock-timeout=%s", i.LockTimeout),
		"-no-color",
		fmt.Sprintf("-plugin-dir=%s -plugin-dir=%s", i.PluginDirs[0], i.PluginDirs[1]),
		"-reconfigure",
		"-upgrade=false",
		"-verify-plugins=true",
		i.Directory,
